...
public class AgentBase : MonoBehaviour
{
    ...
    [SerializeField] float mutationSize = .5f;

    public NN nn;
    public bool gender = false; //false - male; true - female
            ...
            if (hit.collider != null && hit.collider.gameObject != gameObject)
            {
                ...
                else if (hit.collider.CompareTag("agent"))
                {
                    if (hit.distance < forwardAgentNearestDist)
                    {
                        ...
                        genderF = hit.transform.GetComponent<AgentBase>().gender;
                    }
                    Vector3 hit3 = new Vector3(hit.point.x, hit.point.y, origin3.z);
            ...
            if (hit.collider != null && hit.collider.gameObject != gameObject)
            {
                ...
                else if (hit.collider.CompareTag("agent"))
                {
                    if (hit.distance < backwardAgentNearestDist)
                    {
                        ...
                        genderB = hit.transform.GetComponent<AgentBase>().gender;
                    }
                    Vector3 hit3 = new Vector3(hit.point.x, hit.point.y, origin3.z);
                    Debug.DrawLine(origin3, hit3, Color.cyan);
                }
    ...
    public GameObject Copy(GameObject agentPrefab)
    {
        ...
        agentPrefab.GetComponent<NN>().Inicialize(c, node, true, mutationChance, mutationSize, nn.GetInputSize());
        agentPrefab.GetComponent<AgentBase>().gender = (Random.value < .5f) ? true : false;
        ...
        for (int i = 0; i < meshVals.Count; i++)
        {
            if(Random.value < .95f || vals.Count < 4)
            {
                ...
                {
                    vals.Add(meshVals[i] + Random.Range(-mutationSize, mutationSize));
                }
                else
                {
                    vals.Add(meshVals[i]);
                ...
                if (Random.value < mutationChance/2 && vals.Count < 20)
                {
                    vals.Add(Random.Range(-valuesSize, valuesSize));
                }
    ...
    GameObject Crossbreeding(AgentBase a, GameObject agentPrefab)
    {
        ...
        agentPrefab.GetComponent<NN>().Inicialize(connections, nodes, true, mutationChance, mutationSize, nn.GetInputSize());
        agentPrefab.GetComponent<AgentBase>().gender = (Random.value < .5f) ? true : false;
        ...
        for (int i = 0; i < vals.Count; i++)
        {
            if (Random.value < mutationChance)
            {
                vals[i] += Random.Range(-mutationSize, mutationSize);
            }
